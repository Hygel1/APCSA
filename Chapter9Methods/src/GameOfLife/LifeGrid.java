     ...
     */
     LifeGrid(int i, int j){
        ...
        public void evolve(){
            ...
            for(int i=0;i<rtn.length;i++){
                ...
                for(int n=0;n<rtn[i].length;n++){
                    ...
                    boolean cS=gridOfLife[i][n]==ALIVE; //cS==current state in boolean form
                    if(cN==8){ //If any block is fully surrounded, kill everything on screen
                       killAll();
                       brk=true;
                       break;
                    } 
        ...
        public void killAll(){
            ...
            //repaint();
            /*for(int i=0;i<gridOfLife.length;i++){
                for(int n=0;n<gridOfLife[i].length;n++){
                    gridOfLife[i][n]=DEAD;
                }
                //repaint();
            } */
        ...
        private int checkNeighbors(int i,int j){
            ...
            for(int n=i-1;n<i+2;n++){
                int x;
                if(n>-1&&n<gridOfLife.length)x=n; //x-central
                else if(n==gridOfLife.length)x=0; //right-bound, replace all x-values with 0
                else x=gridOfLife.length-1; //left-bound by eliminaion, replace all x-values with rightmost value
                for(int q=j-1;q<j+2;q++){
                    if(q>-1&&q<gridOfLife[0].length) rtn+=gridOfLife[x][q]; //y-central, perform normally
                    else if(q==-1) rtn+=gridOfLife[x][gridOfLife[0].length-1];  //top-bound, consider the bottom value rather than the value above
                    else rtn+=gridOfLife[x][0]; //bottom-bound, consider the top value rather than the value below
                }
                /* less efficient version of rounding the array
