...
public class Trail
{
   ...
   * The number of markers on the trail is markers.length. 
   **/
  private int[] markers; //May refer to markers in any method of the Trail class.

  /** Constructs a new Trail object. */
  public Trail(int[] markers)
  {
    int n = markers.length;
    //note:  this.markers is the FIELD, markers is the parameter
    this.markers  = new int[n]; //make a new array
    for (int i = 0; i < n; i++)
      this.markers[i] = markers[i]; //copy the values of the argument
  }
  ...
  public boolean isLevelTrailSegment(int start, int end)
  {
    ...
    for(int i=start;i<end+1;i++){
      if(high<markers[i]) high=markers[i];
      if(low>markers[i]) low=markers[i];
    }
    return high-low<=10;
  }
  ...
  public boolean isDifficult()
  {
    ...
    for(int i=0;i<markers.length-1;i++){
      if(Math.abs(markers[i]-markers[i+1])>=30) rtn++;
    }
    return rtn>=3;
  }
  ...
  {
	//Sample test code:  build an array, construct a Trail object,
	  //activate non-static methods using dot notation on the constructed
	  //trail.
    int[] elevations = {100, 150, 105, 120, 90, 80, 50, 75, 75, 70, 80, 90, 100};
    Trail t = new Trail(elevations);
    System.out.println("The given trail is: \n" + Arrays.toString(elevations));
    System.out.println("Is level from 7 to 10? " + t.isLevelTrailSegment(7, 10));
    System.out.println("Is level from 2 to 12? " + t.isLevelTrailSegment(2, 12));
    System.out.println("Is it difficult? " + t.isDifficult());
  }
}